...
def get_artists(url, api_key, version, alias):
    """Grab Artist information from the Lidarr endpoint"""

    initial_time = time.time()
    res = util.get(f"{url}/api/{version}/artist", api_key)
    end_time = time.time()
    if res == {}:
        UP.labels(alias, 'lidarr').set(0)
    else:
        for artist in res:
            releases = util.get(
                f"{url}/api/{version}/album?artistId={artist["id"]}",
                api_key)
            track_files = util.get(
                f"{url}/api/{version}/trackfile?artistId={artist["id"]}",
                api_key)
            artist["releases"] = releases
            artist["trackFiles"] = track_files
        UP.labels(alias, 'lidarr').set(1)
        lidarr_metrics.LAST_SCRAPE.labels(alias).set(end_time)
        lidarr_metrics.SCRAPE_DURATION.labels(alias).set(end_time - initial_time)
    return res

def analyse_artists(artists, detailed, alias):
    ...
    lidarr_metrics.ARTIST_COUNT_T.labels(alias).set(len(artists))
    ...
    for artist in artists:
        ...
        if detailed:
            (lidarr_metrics.ARTIST_RELEASE_COUNT.labels(alias, name)
             .set(stats["albumCount"]))
            (lidarr_metrics.ARTIST_TRACK_COUNT.labels(alias, name)
             .set(stats["trackCount"]))
            (lidarr_metrics.ARTIST_DISK_SIZE.labels(alias, name)
             .set(stats["sizeOnDisk"]))
            (lidarr_metrics.ARTIST_MISSING_RELEASE_COUNT.labels(alias, name)
             .set(missing_release_count))
            (lidarr_metrics.ARTIST_MONITORED.labels(alias, name)
    ...
    lidarr_metrics.MONITORED_RELEASE_T.labels(alias).set(release_counter["total"]["monitored"])
    lidarr_metrics.UNMONITORED_RELEASE_T.labels(alias).set(release_counter["total"]["unmonitored"])
    for folder, count in release_counter["path"]["monitored"]["paths"].items():
        lidarr_metrics.MONITORED_RELEASE.labels(alias, folder).set(count)
    for folder, count in release_counter["path"]["unmonitored"]["paths"].items():
        lidarr_metrics.UNMONITORED_RELEASE.labels(alias, folder).set(count)
...
def update_system_data(data, alias):
    ...
    for disk in data["root_folder"]:
        lidarr_metrics.FREE_DISK_SIZE.labels(alias, disk["path"]).set(disk["freeSpace"])
        lidarr_metrics.AVAILABLE_DISK_SIZE.labels(alias, disk["path"]).set(disk["totalSpace"])

    lidarr_metrics.QUEUE_COUNT.labels(alias).set(data["queue"]["totalCount"])
    lidarr_metrics.QUEUE_ERROR.labels(alias).set(data["queue"]["errors"])
    lidarr_metrics.QUEUE_WARNING.labels(alias).set(data["queue"]["warnings"])

    start_time = parse(data["status"]["startTime"]).timestamp()
    build_time = parse(data["status"]["buildTime"]).timestamp()
    lidarr_metrics.START_TIME.labels(alias).set(start_time)
    lidarr_metrics.BUILD_TIME.labels(alias).set(build_time)
...
def scrape(config):
    ...
    alias = config.get('alias', 'lidarr')

    data = get_artists(url, api_key, api_version, alias)
    system = {
...
def update_metrics(data, detailed, alias):
    ...
    analyse_artists(data["data"], detailed, alias)
